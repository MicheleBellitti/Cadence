...
/**
 * Structural types so these converters work with both the Firebase client SDK
 * and the Admin SDK — neither package is imported here, which lets the Cloud
 * Functions connector reuse this module without pulling in the browser SDK.
 */
 ...
 */
export function docToItem(data: DocData, id: string): Item {
  return {
    ...data,
    id,
...
export function docToTeamMember(data: DocData, id: string): TeamMember {
  return {
    ...data,
    id,
...
export function docToSprint(data: DocData, id: string): Sprint {
  return {
    ...data,
    id,
    createdAt: serializeTimestamp(data.createdAt),
...
export function docToOverride(data: DocData, id: string): GanttOverride {
  return {
    itemId: id,
...
export function collectDescendantIds(
  rootId: string,
  allItems: Pick<Item, "id" | "parentId">[]
): Set<string> {
...
export function docToInvite(data: DocData, id: string): Invite {
  return {
    id,
